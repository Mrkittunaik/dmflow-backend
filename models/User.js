...
function encryptToken(plain) {
  ...
  const key    = require('crypto').scryptSync(process.env.TOKEN_SECRET, 'dmflow', 32);
...
function decryptToken(enc) {
  ...
  const key    = require('crypto').scryptSync(process.env.TOKEN_SECRET, 'dmflow', 32);
  const decipher = require('crypto').createDecipheriv('aes-256-cbc', key, Buffer.from(ivHex, 'hex'));
