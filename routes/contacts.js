...
router.get('/export', requireAuth, async (req, res) => {
  try {
    ...
    const rows   = contacts.map(c =>
      [c.username, c.name, c.email, c.dmCount, c.source, c.createdAt?.toISOString()].map(csvCell).join(',')
    );
...
router.patch('/:id', requireAuth, async (req, res) => {
  try {
    const { username, name, avatar, email, tags, notes, optedOut } = req.body;
    const allowed = { username, name, avatar, email, tags, notes, optedOut };
