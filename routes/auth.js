...
router.post('/register', async (req, res) => {
  try {
    ...
    res.status(201).json({ token, user: { _id: user._id, name: user.name, email: user.email, plan: user.plan, avatar: user.avatar, instagram: safeInstagram(user.instagram) } });
  } catch (err) {
...
router.post('/login', async (req, res) => {
  try {
    ...
    const token = makeToken(user);
    res.cookie('dmflow_token', token, cookieOptions());
    res.json({ token, user: { _id: user._id, name: user.name, email: user.email, plan: user.plan, avatar: user.avatar, instagram: safeInstagram(user.instagram) } });
  } catch (err) {
    res.status(500).json({ error: 'Internal server error.' });
  }
});
